...
function MovieEditModal() {
  ...
  useEffect(() => {
    ...
    if (editID) {
      ...
      if (foundedCamera) {
        const miladiDate = new Date(foundedCamera.publishedDate * 1000);
        const formattedDate = miladiDate.toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        });
        console.log("foundedCamera", formattedDate);
        // foundedCamera = {
        //   ...foundedCamera,
  ...
  const handleChangePublishedDate: ChangeEventHandler<HTMLInputElement> = (
    e,
  ) => {
  ...
  const handleChangePublisherName: ChangeEventHandler<HTMLInputElement> = (
    e,
  ) => {
    if (!e.target.value) {
            ...
            >
              <Stack spacing={0.5} style={{ flex: 1 }}>
                <Stack spacing={2} direction={"row"}>
                  <Stack spacing={0.5} style={{ flex: 3 }}>
                    <InputLabel>{t("Select Pdf")}</InputLabel>
                    ...
                    {selectedVideo ? (
                      ...
                      <DropzoneFile
                        validFormats={[".pdf"]}
                        style={{
                          borderColor: Boolean(errors["streamUrl"])
                            ? "red"
                            : "lightgray",
                  ...
                  <Stack spacing={0.5} style={{ flex: 3 }}>
                    <InputLabel>{t("Poster Image")}</InputLabel>
                    {selectedPoster ? (
                      <div className={classes.showFile}>
                        <div className={classes.file}>
                          <img src={imageJpg} />
                      ...
                      <DropzoneFile
                        ...
                        style={{
                          borderColor: Boolean(errors["poster"])
                            ? "red"
                            : "lightgray",
                          // pointerEvents: editID ? 'none' : 'all',
                        }}
              ...
              <Stack spacing={1} direction={"column"}>
                <Stack spacing={2} direction={"row"}>
                  ...
                  <Stack spacing={0.5} style={{ flex: 3 }}>
                    <InputLabel>{t("publishedDate")}</InputLabel>
                    <TextField
                      value={moment
                        ...
                        value.map((option, index: number) => (
                          <Chip
                            ...
                            sx={{
                              mr: 0.5,
                              border: (theme) =>
                                `1px solid ${alpha(theme.palette.info.light, 0.6)}`,
                              background: (theme) =>
                ...
                >
                  <Stack spacing={0.5}>
                    ...
                    <Autocomplete
                      ...
                      size="small"
                      // getOptionLabel={(option:IGenersItem) => option.title}
