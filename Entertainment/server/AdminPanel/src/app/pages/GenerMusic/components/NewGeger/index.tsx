...
function CreateNewGener() {
    ...
    const handleChangeTag = (newValue, reason: AutocompleteChangeReason) => {
        const values: IGenersItem[] = newValue.map(item => typeof item === 'string' ? { title: item } : item);
        switch (reason) {
            case 'createOption':
                const duplicate = geners.findIndex(g => g.title == values.at(-1)?.title)
                const duplicateGenreList = genreList.findIndex(g => g.title == values.at(-1)?.title)
    ...
    const handleSaveCamera = () => {
        ...
        AddGenerList(geners)
            .then(async res => {
                insertGenre(geners.map(g => g.title))
                await window.wait()
                setLoading(false)
