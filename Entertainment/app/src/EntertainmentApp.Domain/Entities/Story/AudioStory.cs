...
{
    public class AudioStory : BaseEntity
    {
        ...
        public List<AudioStoryEpisode> Episodes { get; private set; } = new List<AudioStoryEpisode>();


        private AudioStory() { }
        public AudioStory(string title, string description, List<string> languages, int ageGroup, string posterImageUrl)
        {
            SetTitle(title);
        ...
        public void AddEpisode(AudioStoryEpisode episode)
        {
            if (episode == null)
                throw new DomainException("Episode cannot be null.");
            Episodes.Add(episode);
            episode.SetAudioStory(this);
        }
    }
}
