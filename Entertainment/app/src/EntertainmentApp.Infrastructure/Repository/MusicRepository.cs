namespace EntertainmentApp.Infrastructure.Repository
{
    public class MusicRepository : IMusicRepository
    {
        ...
        public async Task<List<Guid>> GetNotExistTracksId(List<Guid> ids)
        {
        ...
        public async Task<List<Guid>> GetNotExistOnSeverTracksId(List<Guid> ids)
        {
            var serverIds = await _context.Tracks
                .Select(m => m.Id)
                .ToListAsync();
        ...
        public async Task<List<Track>> GetTracksNeedToDownloadAsync()
        {
        ...
        public async Task<List<Album>> GetAlbumsNeedToDownloadAsync()
        {
            var excludedStatuses = new[]
            {
                DownloadStatus.NotNeed,
            ...
            return await _context.Albums.Include(s => s.Episodes)
                .Where(x => !excludedStatuses.Contains(x.DownloadStatus))
                //.Select(x => x.Id)
                .ToListAsync();
        }
        ...
        public async Task<List<Guid>> GetNotExistAlbumsId(List<Guid> ids)
        {
            var existingIds = await _context.Albums
                .Where(m => ids.Contains(m.Id))
                .Select(m => m.Id)
                .ToListAsync();
        ...
        public async Task<List<Guid>> GetNotExistOnSeverAlbumsId(List<Guid> ids)
        {
            var serverIds = await _context.Albums
                .Select(m => m.Id)
                .ToListAsync();
        ...
        public async Task<List<AlbumEpisode>> GetEpisodesNeedToDownloadAsync()
        {
            var excludedStatuses = new[]
            {
                DownloadStatus.NotNeed,
            ...
            return await _context.AlbumEpisodes
                .Where(x => !excludedStatuses.Contains(x.DownloadStatus))
                //.Select(x => x.Id)
                .ToListAsync();
        }
        ...
        public async Task<Album?> GetAlbumByEpisodeIdAsync(Guid episodeId)
        {
            return await _context.Albums
        ...
        public async Task UpdateEpisodeAsync(AlbumEpisode episode)
        {
            _context.AlbumEpisodes.Update(episode);
            await _context.SaveChangesAsync();
        }
