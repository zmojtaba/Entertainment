...
import type { IAudioStory, IEpisodes } from "../types";
...
function Audio_storyPlayer() {
  const params = useParams();
  const movieId = params["id"];
  const navigate = useNavigate();
  const [movie, setMovie] = useState<IAudioStory>();
  ...
  useEffect(() => {
    getMoviesByID(movieId as string)
      .then((res) => {
        const movie: IAudioStory = res.data;
        setMovie(movie);
        setSelectedImage(
    ...
    <div className={classes.container}>
      <div className={classes.header}>
        <div
          ...
          onClick={() => navigate(`/Audio_story/${category}`)}
        >
...
export default Audio_storyPlayer;
