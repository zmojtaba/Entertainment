...
const FluidCanvas = () => {
  ...
  useEffect(() => {
    ...
    function splat(x, y, dx, dy, color) {
      ...
      gl.uniform3f(splatProgram.uniforms.color, dx * 3.0, -dy * 3.5, 1.0);
    ...
    function update() {
      ...
      lastTime = Date.now();
      simTime += dt;
      noiseOffset += dt * 0.5; // سرعت تغییر noise
      ...
      const angleNoise = (Math.sin(simTime * 1.2 + 1.3) + Math.sin(simTime * 2.3 + 2.1)) * 1; // -1 تا 1
      const speedNoise = 1.5 + Math.sin(simTime * 0.8) * 0.8; // پایه 1.5، نوسان ±0.8 → 0.7 تا 2.3

      // جهت پایه: ترکیبی از حرکت عمومی + noise
      let baseAngle = simTime * 0.5; // حرکت کلی چرخشی آهسته
      const directionAngle = baseAngle + angleNoise * Math.PI;

      // سرعت متغیر
      const currentSpeed = 200 * speedNoise; // سرعت پیکسل در ثانیه، تنظیم کن
