...
const FluidCanvas = () => {
  ...
  useEffect(() => {
    ...
    canvas.addEventListener("mousemove", e => {
      const p = pointers[0];
      const dx = (e.offsetX - p.x) * 5.0;
      const dy = (e.offsetY - p.y) * 5.0;
      p.moved = p.down = true;
      p.dx = dx; p.dy = dy;
    ...
    canvas.addEventListener("touchmove", e => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        const dx = (touches[i].pageX - p.x) * 10;
        const dy = (touches[i].pageY - p.y) * 10;
        p.down = true; p.moved = true; p.dx = dx; p.dy = dy;
        p.x = touches[i].pageX; p.y = touches[i].pageY;
        p.color = [Math.random() + 0.2, Math.random() + 0.2, Math.random() + 0.2];
      }
    }, { passive: false });
