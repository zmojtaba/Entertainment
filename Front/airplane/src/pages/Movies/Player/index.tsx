...
function Film() {
  ...
  // console.log('dddddd', `${movie?.streamUrl.replaceAll('\\','/')}`)
  useEffect(() => {
    getMoviesByID(movieId as string).then(res => {
      const movie: Movie = res.data;
      setSelectedImage(`${API_CONFIG.movie}/media/${movie?.posterImageUrl.replaceAll('\\','/')}`)

      if (selectVideoRef.current) {
        selectVideoRef.current.src = `${API_CONFIG.movie}/media/${movie?.streamUrl.replaceAll('\\','/')}`
