...
function ShowMagazin() {
  const params = useParams()
  const movieId = params['id']
  const navigate = useNavigate()
  const [movie, setMovie] = useState<IMagazin>();
  const [play, setPlay] = useState(false);
  const [selectImage, setSelectedImage] = useState('');
  const [selectpdf, setSelectedPdf] = useState('');
  // console.log("ffffff : ",selectpdf);

  const defaultLayoutPluginInstance = defaultLayoutPlugin();

  useEffect(() => {
    getMoviesByID(movieId as string).then(res => {
      const movie: IMagazin = res.data;
      // console.log('Imagazin', movie)
      // console.log('Imagazin', `${API_CONFIG.movie}/media/${movie?.streamUrl.replaceAll('\\', '/')}`)
      ...
      // selectVideoRef.current.src = `http://10.211.47.233:5030/media/${movie?.streamUrl.replaceAll('\\', '/')}`


      // window.open('http://10.211.47.233:5030/media/publication/magazine/بیللیبل/c6336dc442de42f6b180342c5295bb02_MyResume-31[www.cvbuilder.me]574.pdf','_blanck')
    }).catch(_err => {
