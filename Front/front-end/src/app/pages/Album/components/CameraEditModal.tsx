...
import ArrowRightRoundedIcon from '@mui/icons-material/ArrowLeftRounded';
...
import { uploadVideo, updateVideo, EditAlbum } from '../constants/api';
...
function MovieEditModal() {
    ...
    const handleSaveCamera = () => {
        ...
        if (!movie.singer.name) {
            temErrors.singer = 'select singer'
        }
        ...
        if (!isError) {
            ...
            if (!editID) {
                ...
                EditAlbum(movie, selectedPoster!).then(res => {
                    insertMovie(res.data)
                    setLoading(false)
                    handleClose()
                }).catch(err => {
    ...
    return (
        ...
        >
            ...
            <DialogTitle sx={{ display: 'flex', alignItems: 'center', py: .5, pr: 1, fontSize: '1.4rem' }}>
                <p>
                    {editID ? 'Edit Album' : t('New Album')}
                </p>
                {editID &&
                    <p style={{ fontSize: '1.6rem', fontWeight: 600 }} >
                        <ArrowRightRoundedIcon color="action" fontSize="large" />  {MovieFirstValue?.current?.title}
                    </p>
                }
            ...
            <DialogContent dividers
                ...
                }}>
                {
                    errorLoadingMovies || notFoundError ?
                        <ErrorComponent
                            error
                            errorMessage={t(errorLoadingMovies ? 'Failed load Album list' : 'Album not found')}
