...
import { DataRefrence, IGenersItem } from "../store/type";
...
// -----------------------------------
export const uploadVideo = (movie: IMovie, file: File, poster: File, onProgress) => {
    ...
    formData.append('Languages', JSON.stringify(movie?.language.map(l => l)));
...
export const updateVideo = (movie: IMovie) => {
    ...
    const mapped = {
        ...
        languages: movie.language.map(l => l),
